...
class TestIfThenElse extends FlatSpec with Matchers with GivenWhenThen with BeforeAndAfter {
  ...
  it should "lookup(result) must be equal to 5" in {
    ...
    val a4 = new Assignment("result", new AddExpression(new VarRef("val5"), new VarRef("val10")))
    val a5 = new Assignment("result", new SubExpression(new VarRef("val10"), new VarRef("val5")))
    val cond = new GrExpression(new VarRef("val5"), new VarRef("val10"))
